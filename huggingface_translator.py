"""
Hugging Face-based English to Ibani translator using ByT5.
This implements training and inference for neural machine translation.
ByT5 works at the character/byte level, making it ideal for Ibani's special characters.
"""
...
class IbaniHuggingFaceTranslator:
    """
    English to Ibani neural machine translation using Hugging Face ByT5.
    
    This class provides training and inference capabilities for translating
    English text to Ibani language using a fine-tuned ByT5 model.
    ByT5 works at the byte/character level, preserving special characters like á, ḅ, etc.
    """
    ...
    def __init__(self, model_name: str = "google/byt5-small", 
        ...
        Args:
            model_name: Pre-trained model to use as base (default: google/byt5-small)
            model_path: Path to local fine-tuned model (if available)
            hf_repo: HuggingFace Hub repository (e.g., "username/ibani-byt5-translator")
        ...
        # Try HuggingFace Hub if local doesn't exist
            ...
            try:
                ...
                self.tokenizer = ByT5Tokenizer.from_pretrained(hf_repo)
                self.model = T5ForConditionalGeneration.from_pretrained(hf_repo)
    ...
    def preprocess_data(self, examples):
        """Preprocess data for training with ByT5 task prefix."""
        # Add task prefix to inputs for ByT5
        inputs = [self.task_prefix + self.normalize_text(ex["en"]) for ex in examples["translation"]]
        targets = [self.normalize_text(ex["ibani"]) for ex in examples["translation"]]
        
        # Tokenize inputs
        model_inputs = self.tokenizer(
            inputs, 
            max_length=256,  # Increased for character-level tokenization
            truncation=True, 
            padding=True
        )
    ...
    def translate(self, text: str, use_fallback: bool = False) -> str:
        ...
        # Tokenize input with task prefix for ByT5
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                max_length=256,  # Increased for character-level output
                num_beams=5,  # Slightly increased for better quality
                early_stopping=True,
